...
optimal} elements of a set. This is also known as maximum vector
...
% Chris: Muessten nach obiger Definition ("dominated by _at most_ K points") 
% nicht
% $\skyband_\dominates^1 = \{r, s\}$ und
% $\skyband_\dominates^2 = \{r, s, t\}$ gelten?
...
Let $rel$ be a relation with the attributes $a_1, \ldots, a_n$, a non
...
\inlinesql{SKYLINE~OF}-clause the situation is changed, such a query has
following schema:
%
...
\bigskip
Matters get more involved in the first case.  If we can establish an
...
their attribute from the same tuple, which can be expected as the same
...
Anyway it is somewhat questionable what it is good for to include a
...
The task is here to initially compute the skyline and then update the
