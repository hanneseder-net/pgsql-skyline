...
quality and a cheap product with lower qualtity.  For sure we are not
...
anothers}, as there are others which better fulfill our criteria.  The
...
descision.  These products are called to be \emph{pareto optimal}
...
pseudocode to convey the essence of the algorithms more concisely.

For the presentation we adapt the convention in \citep[Page
19]{Cormen2001}, in the very essence i.e.:

\begin{itemize}
\item No begin/end or curly brackets for blocks, we use \emph{indentation}
to express the \emph{block structure}, like in languages such as Python.

\item We do explicitly \emph{break} at the end of case statements, because
in some rare situations we \emph{fall through} to the next case.
\end{itemize}
...
\todo{see \citep[section 1.1.1]{Simkovics1998} }{}
...
\end{proof}
Since the skyline operator is a special case of \emph{winnow} operator, \autoref{the:skyline-idempotent} holds as well.
...
The \nospellcheck{$n$-th} skyline startum $\skyline_C^n$ is recursively defined as:
...
standards. \todo{restaurant, close, good, tiered of eating there}{}
...
\fixme{It is easy to see, that the following identities hold}{that's wrong}
        x3
  x1  
...
0-skyband, x3 is not is 1-skyband because it is dominated by x1 and
...
\subsection{Monotone vs. linear scoring functions}\label{sec:scoring}
...
distance of hotels, you will find your favorite hotel in the Skyline.
...
(cf. \autoref{def:skyline}) and with the definition of the binary
...
intended semantics of \inlinesql{SKYLINE~OF DISTINCT} is
...
least one tuple.}.  In the presents of a unique and ordered
key\footnote{On some RDBMS special pseudocolumns for such proposes do
...
For the border case of one dimensional skyline queries, i.e. $m = 1$,
...
In particular, the convex hull contains the subset of skyline points
...
skyline algorithms can only be placed an the leaves of an algebraic
expression, where the non-indexed based skyline algorithms are first
...
\subsubsection{Non-termination of Block-Nested-Loops (BNL) Algorithm}
...
an explicit sort can be omitted or for instance consider different
physical operators for aggregation or joining.
...
is guaranteed to survive the elimination filter.  An other notable
...
\section{Continuous Skyline Computation}
This is a whole other class of skyline algorithms, the task is here
the initially compute the skyline and that update the skyline of tuples
...
\autoref{sec:relative-tuple-order}.  To work around this reordering if
...
same. While the latter clearly indicates a flaw in the implementation.
...
semantics this definition gave as maximum flexibility during
...
aspects of the skyline operator semantics which are not cleare through
the basic definition.  The semantics of the options for the
\inlinesql{SKYLINE~OF}-clause is either already given in the describtion
of the syntax in \autoref{sec:syntax} or in the appropriate subsection
in the describtion of query execution (cf. \autoref{sec:queryexecution}).
...
literature, but a real implemenation in a database system of course
must be able to cope with null values.  Furthermore we have a syntax
to specify how to deal with null values.  Once again this is a feature
...
the user the specify how to treat null values.  When sorting ascending
per default null values sort after non-null values and when sorting
descedning null values sort before non-null values, as above see the
...
the null values before the non-null values.  Specifing
\inlinesql{NULLS FIRST/LAST} overrides this default.  The question
may arise if \inlinesql{NULLS LAST} should be the default for
\inlinesql{SKYLINE OF MAX}, but as this is not the case for the
\inlinesql{ORDER BY DESC} we stick with \inlinesql{NULLS FIRST} to
be consistent.
...
\inlinesql{GROUP~BY}-clause more limitations apply, which immediatly
...
important role throughout the query plan.  A note within the query plan
...
suitable interface for this iterator whould just expose these methods:
...
just add an approriate case in the these dispatch function and call
our own code:
...
something in each state and then if neccessary proceed to another
...
\todo{lookup make skyline pathkey, what is the name of the function that matches a pathkey against a skyline pathkeys}{}
...
\texttt{skyline\_method\_can\_use\_limit}\footnote{in \srcref{src/backend/utils/skyline/skyline.c}}
...
\todo{write about slot, resultSlot that they are preserved over calls
		...
		case PROCESS:
			...
			forever do begin
				if tuplewindow_ateof($state.window$) then begin
					if tuplewindow_hasfreespace($state.window$) then
...
begin
	forever do begin
		...
		case PROCESS:
			...
			forever do begin
				if tuplewindow_ateof($state.window$) then begin
					if tuplewindow_hasfreespace($state.window$) then begin
						tuplewindow_put($state.window$, $slot$);
						return $slot$;
					end
					else begin
...
and this would be appreciated as part of a checked build.  For the
...
\verb|seqscan{.ALIAS}.on|		& Name of the base table, when an alias is used for the table it is given as \texttt{.ALIAS}, like when we express a skyline query only with SQL, we have an inner \texttt{.i} and an outer \texttt{.o} relation.\\
...
\section{Different syntax for \inlinesql{SKYLINE OF DIFF}}
...
\subsection{Order in which Columns are compared}
...
histograms for the base relations be used for?  Or e.g. Compare
integer data before strings.
...
\section{Speedups for low cardinality domains}
%
use special algorithms if we deal with domains with small
cardinalities, \citep{Preisinger2006, Preisinger2007, Morse2007}
...
For SFS differentiate between CMP\_INCOMPARABLE\_SAME\_GROUP and
CMP\_INCOMPARABLE\_DIFFERENT\_GROUP.
...
could be engaged. \todo{cite}{Chaurduri2006}
...
% LocalWords:  Tafelspitz startum dataset Papadias skybands unnested Wirth yacc
