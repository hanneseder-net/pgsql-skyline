...
\newcommand\nondistinct{\ensuremath{\triangleq}\xspace} % todo: find another symbol
...
pairs of tuples from the same (database) relation, we say $r$
\emph{dominates} $s$ in \dominates iff $(r, s) \in \dominates$ (or in
infix notation $r \dominates s$).
...
\end{lstlisting}
\caption{Pseudo code for Original Block-Nested-Loops (BNL) Algorithm}
\label{code:bnl}
\end{figure}
...
$n=m=1$. Nevertheless it is not very had to fix this issue, a slight
modification in the control flow solves the problem, see
Figure~\ref{code:fixedbnl}, and of course our implementation into
PostgreSQL is based on our fixed version.
