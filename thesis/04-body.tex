...
SQL's \inlinesql{EXCEPT}\index{SQL!EXCEPT@\texttt{EXCEPT}} (see \autoref{sql:rewrite-with-except}).
...
the alias $o$.
%
Another way\index{SQL!EXCEPT@\texttt{EXCEPT}} to rewrite the query is given in
...
\inlinesql{USING}~\emph{qualOp}. We call the latter specification 
...
FIRST} and \inlinesql{NULLS LAST}.  We have the same concept for the
...
operator as well.  In the \autoref{sec:bnl-tuple-order},
...
gave ourself the freedom to present the tuple store\index{tuple store} interface
...
\autoref{code:2dimpresort}.  Of course in the presence of an
...
the skyline selectivity\index{selectivity} factor\footnote{selectivity factor = skyline
