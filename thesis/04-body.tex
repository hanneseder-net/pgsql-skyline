...
decision.  These products are called \emph{Pareto optimal}\index{Pareto optimal}
...
SQL's \inlinesql{EXCEPT}\index{EXCEPT (SQL)@\texttt{EXCEPT} (SQL)}
(see \autoref{sql:rewrite-with-except}).
...
RDBMS\index{RDBMS} context, with the ultimate goal of building a
...
%
We propose a new use case for the elimination filter (EF) and
show its usefulness through experiments, namely: BNL+EF.
%
