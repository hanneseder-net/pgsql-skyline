...
\index{multi-criteria optimization}
...
decision.  These products are called \emph{\idx{Pareto optimal}}
based on the given criteria.
%
For this notion \citet{Borzsonyi2001} coined the term \emph{skyline}.
%
The associated relational operator is called \emph{skyline operator}.

%FIXME: reinhard: what queries.
...
\citet{Borzsonyi2001} proposed a \emph{skyline} extension of the SQL
syntax with this form:

\idxonlysql{SKYLINE OF}
\begin{sql}
SELECT ... FROM ... WHERE ... \\
GROUP BY ... HAVING ...       \\
...
Computing the skyline is also known as the \emph{\idx{maximum vector
...
\idxsql{DIFF} and \inlinesql{DISTINCT} keywords. \idxonlysql{SKYLINE OF DISTINCT}
...
\noindent
In the above query $a$ and $b$ will be part of the answer, whereas in the following one
it is up to the database system to make a choice between $a$ or $b$, as they 
are equally good:

\idxonlysql{SKYLINE OF DISTINCT}
\begin{interactive}
...
The \emph{skyline operator} \citep{Borzsonyi2001} is within the expressive
power of SQL and in \autoref{sec:rewrite-to-sql} we show how to
rewrite an SQL query with a \inlinesql{SKYLINE~OF}-clause into
standard SQL.
%
However there are good reasons for including a \emph{skyline} or 
\emph{preference operator} into SQL and building it into an RDBMS:
...
SQL's \idxsql{EXCEPT} (see \autoref{sql:rewrite-with-except}).

\item
It is faster to evaluate, as there are numerous algorithms for skyline
queries. Furthermore when this preference query is expressed with a
correlated subselect it boils down to a \naive ``nested-loop''.
...
\item
The skyline operator can be integrated with other relational
operators, thus more sophisticated queries can be constructed.
Moreover, no work on concurrency control and transaction management
has to be addressed.

\item
The existing index structures such as R-trees in an RDBMS can be
adapted to implement index-based skyline algorithms.

\item
If several skyline algorithms are available in an RDBMS, the system is
able to select the most efficient one for the given datasets.
\end{itemize}

It is well known that the cost estimation of the skyline queries is a
non-trivial task \citep{Chaudhuri2006} since the performance of a
skyline query is sensitive to a number of parameters
\citep{Godfrey2007}.
%
Hence building a skyline query optimizer is a  challenging task.


\section{Goal and Main Results} 

In this thesis we aim at extending PostgreSQL with the skyline
operator and the evaluation of skyline algorithms in the
\idx{RDBMS} context, with the ultimate goal of building a
skyline query optimizer to automatically generate a good query plan
w.r.t. I/O, time, and memory consumption.

We wish to acquaint the skyline operator to a broader audience and lay
the ground for future work in this area by providing an open source
implemenation in an RDBMS, which is the natural habitat for the
skyline operator.

The scope of this thesis is restricted to non-index-based skyline
algorithms.
%
Dedicated algorithms have been proposed for the case where the whole
relation fits into main memory \citep{Preparata1985}, nevertheless we
do not take them into account as memory is always a rare resource in a
database scenario, either due to the number of concurrent users or
limited total main memory, e.g. on a handheld device.

\bigskip
\noindent
So far we have implemented the non-index-based algorithms \emph{Block
Nested Loop} (BNL), \emph{Sort First Skyline} (SFS), and a variant
of \emph{Linear Elimination Sort for Skyline} (LESS) in PostgreSQL.
%
...
%
In addition, we extend the standard syntax to specify the following
aspects:
\idxonlysql{USING}
\begin{itemize}
\item The treatment of NULL values (\idxsql{NULLS FIRST} and \idxsql{NULLS LAST})
...
\item \emph{usage of indexes} (\idxsql{NOINDEX})
\item \emph{usage of elimination filter (EF)}
\end{itemize}
\end{itemize}

We prove that the original version of BNL does not terminate in all
cases and give a corrected version.

To achieve the goal of building a skyline query optimizer, we
conducted extensive experiments on the skyline operator
implementations.
%
The experimental environment we have designed and set up is noteworthy
on its own.
%
Our experiments confirm the result from \citet{Borzsonyi2001}, that
specialized algorithms are faster than the same queries in standard
SQL by orders of magnitude.

The comparisons of the skyline algorithms we have so far implemented
reveal that there does not exist a clear winner in all aspects.  The
efficiency of the algorithms depends on the properties of the datasets
such as dimension, distribution, cardinality and so on.

However we discovered several \emph{hidden rules}, which are
remarkably simple and useful, but hard to obtain from the theoretical
investigation.  We expect that our exposition of experimental results
on skyline algorithms could serve as a guideline for developing
heuristics of a skyline query optimizer, and in the meantime provide
some insight for a deeper understanding of the skyline query
characteristics.

\begin{itemize}
\item
For instance, the elimination filter is effective only if the
selectivity factor for the skyline query is not more than 0.1, and

\item
BNL is more efficient than SFS and even LESS, if the dimension of the
data is less than 5 and the number of tuples is relatively small.
\end{itemize}

For the purpose of repeatability we have set up a website to present
our experimental results. The source code for our implementation and
the log-files from our experiments are online accessible at this
website. Furthermore it is possible to test-drive our implementation
through a web-interface.  All this is available at:
...
\index{dollar sign@\texttt{\$} (shell prompt)}
\index{db=\#@\texttt{db=\#} (\texttt{psql} prompt)}
...
We are in the realm of the \idx{relational model} of
data.  As we aim at an implementation of the skyline operator into a
relational database management system (\idx{RDBMS}) we restrict ourselves
to \idx{finite database} instances.
...
it is not necessary for this thesis. For $N$ we require that \idx{equality}
($=$), \idx{inequality} ($\not=$), and the binary relations $<$ (\idx{strictly less than}), 
$\le$ (\idx{less than or equal}), $\ge$ (\idx{greater than or equal}),
and $>$ (\idx{strictly greater than}) are defined with the usual properties.

As the skyline operator is a special case of the \emph{\idx{winnow operator}}
\citep{Chomicki2003a}, we define it in a very similar way.

\begin{definition}[Preference Relation \citep{Chomicki2003a}]
\index{preference relation}
\index{dominates@$\dominates$|see{preference relation}}
...
We require the relation \dominates to be a \emph{\idx{strict partial order}}, 
i.e. \dominates is irreflexive,
asymmetric and transitive. These properties are formalized as usual:

\begin{itemize}
\item \emph{\idx{irreflexivity}:} 
$\forall x: \neg (x \dominates x)$

\item \emph{\idx{asymmetry}:} 
$\forall x, y: x \dominates y \imp \neg (y \dominates x)$

\item \emph{\idx{transitivity}:} 
$\forall x, y, z: (x \dominates y \land y \dominates z) \imp x \dominates z$

\end{itemize}

\noindent
The properties for the corresponding \emph{\idx{partial order}}
$\weakdominates$ are formalized as:
\index{weak preference}
\index{weak dominates@$\weakdominates$|see{weak preference}}
\begin{itemize}
\item \emph{\idx{reflexivity}:} 
$\forall x: x \weakdominates x$

\item \emph{\idx{anti-symmetry}:} 
...
\begin{itemize}
\item \emph{\idx{totality}:} $\forall x, y: x \dominates y \lor y \dominates x \lor x = y$,
\end{itemize}

\noindent
then it is a \emph{\idx{total order}}.
...
% Non-transitive preferences can be compute with
...
\index{intrinsic|see{preference formula}}
\index{extrinsic|see{preference formula}}
...
\index{skyline preference formula}
...
case of the \emph{\idx{winnow operator}} \citep{Chomicki2003a}, where the
...
\noindent
If $r \nondistinct s$ and in case of \idxsql{SKYLINE OF DISTINCT}, then it
...
\noindent
\index{dominated by@$\dominatedby$|see{preference relation}}
\index{weak dominated by@$\weakdominatedby$|see{weak preference}}
For convenience we define the following two \emph{commutator} relations\index{commutator relation}:
\[
...
\begin{definition}[Incomparable]
\index{incomparable}
\index{incomparable@$\incomparable$|see{incomperable}}
If neither $r$ dominates $s$ nor $s$ dominates $r$ then $r$ and $s$
...
\index{comparable}
\index{comparable@$\comparable$|see{comparable}}
The tuples $r$ and $s$ are \emph{comparable} iff $r$ dominates $s$ or
$s$ dominates $r$:
\[
...
\index{SKYLINE OF DIFF@\texttt{SKYLINE OF DIFF} (SQL)|see{\texttt{DIFF} (SQL)}}
...
As noted in \citep{Chomicki2003}, the \idxsql{DIFF} directive works
like a \idxsql{GROUP BY} within the skyline, and the skyline for
...
% operator} \citep{Chomicki2003a}, where the
% preference formula has exactly the form of formula
% \eqref{equ:skylinepf}.
%
...
\index{monotone scoring function}
...
\index{independence of attribute order}
...
\index{skyline stratum|see{stratum}}
\index{K-skyband@$K$-skyband}
In \citep{Chomicki2003a} the concept of skyline stratum is called
\emph{\idx{iterated preference}}, and they speak of 
\emph{\idx{ranking}}, we prefer to use the term
...
the set of ``\idx{second-best}'' tuples.
%
...
%
\idxonlysql{EXCEPT}
Another way to rewrite the query is given in \citep[Page~3]{Godfrey2004}:
...
without those tuples that have a \emph{\idx{witness}} that
...
key\footnote{On some \idx{RDBMS} special pseudo-columns\index{pseudo-column} for such purposes do
exist.} this is obvious, in the other case as long as on each
attribute a total order exists, we can construct a \idx{lexicographic
...
just like to sketch\footnote{We thank Chris Roschger\index{Roschger, Chris}.
...
the same access path will be used.  The \idxsql{LIMIT}-clause is
used, which is not part of the \idx{SQL 2003 standard} \citep{SQL2003}. We conjecture that
...
\begin{enumerate}
\item
\emph{Progressiveness}: \index{progressiveness}
the first results should be reported to the user almost instantly and
the output size should gradually increase.

\item
\emph{Absence of false misses}: \index{absence of false misses}
given enough time, the algorithm should generate the entire skyline.

\item
\emph{Absence of false hits}: \index{absence of false hits}
the algorithm should not discover temporary skyline points that will
be later replaced.

\item
\emph{Fairness}: \index{fairness}
the algorithm should not favor points that are particularly good in
one dimension.

\item
\emph{Incorporation of preferences}: \index{incorporation of preferences}
the users should be able to determine the order according to which
skyline points are reported.

\item
\emph{Universality}: \index{universality}
the algorithm should be applicable to any dataset distribution and
dimensionality, using some standard index structure.
\end{enumerate}
...
attribute, as long as a \emph{\idx{sort function}} is defined for the domain
...
e.g. \idxsql{VARCHAR} column in a skyline query.
Nevertheless this might be a good use case for a \idx{user-defined ordering operator}.

Furthermore our implementation allows arbitrary expressions instead of
a single attribute, e.g.

\idxonlysql{SKYLINE OF}
\idxonlysql{NULLS LAST}
\begin{sql}
...
\idx{convex hull}, \emph{top-K} queries\index{top-k queries}, and \index{nearest-neighbor search}
...
number of \idx{secondary-memory algorithms} have been developed for
...
\emph{Block Nested Loops} (BNL) \citep{Borzsonyi2001}, 
\emph{Divide and Conquer} (D\&C) \citep{Borzsonyi2001}, \index{Divide and Conquer (D\&C) (algorithm)}
\emph{Sort First Skyline} (SFS) \citep{Chomicki2003}, and
\emph{Linear Elimination Sort for Skyline} (LESS) \citep{Godfrey2005}
...
\emph{Bitmap} \citep{Tan2001}, \index{Bitmap (algorithm)}
\emph{Index} \citep{Tan2001}, \index{Index (algorithm)}
\emph{Nearest Neighbor} (NN) \citep{Kossmann2002}, and \index{Nearest Neighbor (NN) (algorithm)}
\emph{Branch and Bound} (BBS) \citep{Papadias2003, Papadias2005}. \index{Branch and Bound (BBS) (algorithm)}
...
expression, whereas the non-index-based skyline algorithms are first
...
\emph{\idx{continuous skyline} computation}.  
...
SFS+EF. EF stands for \emph{Elimination Filter}, which was introduced
...
BNL \citep{Borzsonyi2001} works as follows.  A \emph{\idx{tuple window}} is
...
\ldots, a_n, b_1, \ldots b_m$ \idx{anti-correlated} and let the $c_i$'s just be
...
\idx{topologically sorted} at start-up time.  It is shown in the original
...
%\begin{figure}[htbp]
\begin{lstlisting}[language=pseudo,
caption={Elimination Filter (EF) (in iterator style)},
label={code:ef}
]
...
implementation of the skyline operator into an \idx{RDBMS} as a full-blown
relational operator.
...
cost-based optimizer.  The syntax they are using is \idx{Preference~SQL} 
\citep{Kiessling2002a}.


\citep{Goncalves2005} extended PostgreSQL with a hybrid approach 
between \emph{top-k} and \emph{skyline operator} called \idx{top-k-skyline}.
...
nevertheless the \idx{selectivity} of this approach is even lower than
...
\item Operational aspects of skyline computation, such as 
\begin{itemize}
\item \emph{method} (\idxsql{BNL}, \idxsql{SFS}, \idxsql{MNL}, \idxsql{PRESORT} (2 dim only))
\item \emph{tuple window size} in terms of memory and/or number of slots
\item \emph{tuple window policy} (append, prepend, ranked by entropy, ranked by a random value)
\item \emph{usage of indexes} (\idxsql{NOINDEX})
\item \emph{usage of elimination filter (\idxsql{EF}), 
...
\index{railroad diagrams}
\index{syntax diagrams|see{railroad diagrams}}
\label{sec:syntax}
...
\idx{Backus-Naur form} or \idx{EBNF} and they have been made popular by books such
%done:fang: Railroad _diagram is_ ....  _it has_
%hannes: I think the plural is correct here.
as the ``Pascal User Manual'' written by Niklaus Wirth\index{Wirth, Niklaus}.  Nowadays
...
\nt{\idx{from\_clause}}, \nt{\idx{where\_clause}}, \nt{\idx{group\_clause}},
\nt{\idx{having\_clause}}, \nt{\idx{sort\_clause}}, and \nt{\idx{select\_limit}} have
...
is because a similar concept (\idxsql{ASC}/\idxsql{DESC}) in the
\inlinesql{ORDER~BY}-clause is also called direction.  Furthermore the
treatment of null values can be specified using \idxsql{NULLS FIRST}
and \idxsql{NULLS LAST}.  We have the same concept for the
\idxsql{ORDER BY}-clause. This special treatment of null values 
...
\index{skyline\_of\_expr}
\idxonlysql{MIN}
\idxonlysql{MAX}
\idxonlysql{DIFF}
\idxonlysql{USING}
\idxonlysql{NULLS FIRST}
\idxonlysql{NULLS LAST}
\begin{rail}
skylineofexpr:
...
outer query plan node in a \idx{tuple store}%
\footnote{Tuple store is PostgreSQL concept for temporary files
...
\index{ef\_window\_options}
\idxonlysql{EFSLOTS}
\idxonlysql{EFWINDOW}
\idxonlysql{EFWINDOWSIZE}
\idxonlysql{EFWINDOWPOLICY}
\begin{rail}
efwindowoptions : 
    ( 'EFSLOTS' '=' slots) ? \\
...
\index{window\_options}
\idxonlysql{SLOTS}
\idxonlysql{WINDOW}
\idxonlysql{WINDOWSIZE}
\idxonlysql{WINDOWPOLICY}
\begin{rail}
windowoptions : 
    ( 'SLOTS' '=' slots ) ? \\
...
\noindent
For the \inlinesql{SKYLINE OF} clause we decided that the it should be
\idx{left associative}, i.e.
%
\begin{sql}SELECT * FROM foo UNION SELECT * FROM bar SKYLINE OF baz\end{sql}
will be parsed as 
\begin{sql}SELECT * FROM foo UNION (SELECT * FROM bar SKYLINE OF baz)\end{sql}

\noindent{}We did so, because we believe the \idxsql{SKYLINE OF} clause is closer
related to the \idxsql{GROUP BY} clause than to the \idxsql{ORDER BY}
clause, therefore we parse it in the same way.

\subsection{Reserved Keywords}
\index{keywords}
\index{reserved keywords|see{keywords}}
...
We did not define \idxsql{MIN}, \idxsql{MAX}, and
\idxsql{DIFF} as reserved keywords, because the SQL aggregate
...
the \inlinesql{ORDER~BY}-clause, where \idxsql{ASC} is equivalent to
\inlinesql{USING <} and \idxsql{DESC} is equivalent to \inlinesql{USING >}, 
...
the keywords \idxsql{NULLS FIRST} and \idxsql{NULLS LAST} allows
the user the specify how to treat null values.  
If sorting ascending (\idxsql{ORDER BY} \emph{expr}
\idxsql{ASC}), then the default is to sort null values \emph{after}
non-null values.
%
The default for \inlinesql{ORDER BY} \emph{expr} \idxsql{DESC}, i.e. sort
...
%done:fang: "Question may arise ..."
%hannes: I think "The question may arise ..." is correct
The question
...
\subsection{\idxsql{SKYLINE OF} and \idxsql{GROUP BY}}
...
or \emph{order of tuples}, but in an \idx{RDBMS} the relative order of
...
on how to treat NULL values (cf. \idxsql{NULLS FIRST}/\idxsql{NULLS LAST}).
The information if the sort order is ascending (\idxsql{ASC})
or descending (\idxsql{DESC}) is encoded in the sort operator used, as
...
PostgreSQL's query execution engine has a top down driven \idx{pipelined
...
suitable interface for this \idx{iterator} would just expose these methods:
...
just add an appropriate case in the these \idx{dispatch functions} and call
your own code:
...
\noindent
Each execution node is coded in the fashion of a \idx{state machine}.  Do
something in each state and then if necessary proceed to another
state.  The whole state is stored in the node's \idx{execution state}
...
\idx{simple object system} with support for \idx{single inheritance}.
The C structure \texttt{Node} is the root of the \idx{class hierarchy}.
...
stage is called \emph{\idx{parse tree}}.
...
from the previous stage is \idx{semantically analyzed}, the outcome is
called \emph{\idx{query tree}}.  Look-ups into the system catalog are done in
...
For our implementation it was not necessary to modify the \emph{\idx{query
rewriting}} stage.  For details see documentation on the
\postgresdocu{rule-system.html}{PostgreSQL Rule System}. \index{rule system}

\section{Query Planning/Optimizing}
The \emph{query tree} from the previous stage is transformed into a
\emph{\idx{plan tree}} during this stage.  Usually there are different
...
precise we must speak of \emph{\idx{pathkeys}}, i.e. both of
...
must not be flattened out\index{flatten out queries}, i.e. do not pull up the subquery.  The function
...
\emph{\idx{pathkeys}} as described in \autoref{sec:pathkeys}. As the
...
\idx{anti-correlated} tuples show up in the tuple stream.  The tuple window
...
\index{query plan interpretation}
...
all minimal tuples in a \idx{tuple store}.  For
...
gave ourselves the freedom to present the \idx{tuple store} interface
...
%\begin{figure}[htbp]
\begin{lstlisting}[language=pseudo,
caption={Special case: 1 dimensional (in iterator style)},
label={code:1dim}
]
...
The \idx{SQL 2003 standard} \citep{SQL2003} does not provide a way
to limit the size of a result set. Nevertheless PostgreSQL and other
\idx{RDBMS} do provide syntax for this and consider it during query
...
\index{pseudo-column}
In Oracle the pseudo-column \idxsql{ROWNUM}
...
%\begin{figure}[htbp]
\begin{lstlisting}[language=pseudo,
caption={Special case: 1 dimensional distinct (in iterator style)},
...
\noindent
\idxonlysql{SKYLINE DISTINCT OF}
...
%\begin{figure}[htbp]
\begin{lstlisting}[language=pseudo,
caption={Special case: 2 dimensional with presort (in iterator style)},
...
compare every tuple to all others as long as no \idx{witness} is found, which
...
%\begin{figure}[htbp]
\begin{lstlisting}[language=pseudo,
caption={Materialized Nested Loop (MNL) (in iterator style)},
label={code:mnl}
]
...
A \idx{tuple window} is a data structure used by BNL, SFS and EF to store
...
\begin{itemize}
\item \lstinline|TupleWindowState *tuplewindow_begin(int maxKBytes, int maxSlots, | \prebreak \\%
\postbreak \lstinline| TupleWindowPolicy policy)|
creates a tuple window. The maximum size of the tuple window is 
limited to \lstinline|maxKBytes|
KB or \lstinline|maxSlots| slots. The number of
...
elimination filter. We speak of a \emph{\idx{forced insert}}.
Currently the \lstinline|timestamp| argument is only used
...
removes the current tuple.  Note that this modifies the cursor.
...
tuple window.  This is not the same as calling
...
\begin{enumerate}
\item \emph{append:} \index{append|see{placement policies}}
a new tuple is always inserted at the \emph{end} of the list

\item \emph{prepend:} \index{prepend|see{placement policies}}
instead of the end, new tuples are inserted at the \emph{beginning} of
the list

\item \emph{entropy:} \index{entropy|see{placement policies}}
...
\item \emph{random:} \index{random|see{placement policies}} 
...
%\begin{figure}[htbp]
\begin{lstlisting}[language=pseudo,
caption={Block Nested Loop (BNL) (in iterator style)},
...
\begin{lstlisting}[language=pseudo,
caption={Sort First Skyline (SFS) (in iterator style)},
...
the concept of \emph{elimination filter} (EF). With the original LESS
...
physical sort operator, we implemented the elimination filter \emph{
...
\index{Concurrent Version System (CVS)}
...
for this, just operate the PostgreSQL in \emph{\idx{single user
mode}}.  This gives one a prompt, to enter
queries directly and the query is evaluated in the same process.  So
debugging or, with Microsoft Visual C++, even \emph{\idx{edit \&
...
distribution type (\idx{independent}, \idx{correlated}, and \idx{anti-correlated}), and
...
\emph{\idx{independent}}, \emph{\idx{correlated}}, and \emph{\idx{anti-correlated}}.
...
%
\noindent
\index{random\_equal@\lstinline{random_equal()}}
...
\noindent
\index{random\_peak@\lstinline{random_peak()}}
...
\index{random\_normal@\lstinline{random_normal()}}
...
\begin{itemize}
\item \emph{indep}: \index{independent}
...
right of the grayscale \idx{density plot} is the \idx{border distribution} for
each dimension, in this plots the blue line indicates the density of a
\idx{normal distribution} with the same mean and standard deviation as the
border distribution.
...
\item \emph{corr}: 
a \idx{correlated} dataset represents an environment in which points which
...
\item \emph{anti}:
an \idx{anti-correlated} dataset represents an environment in which points
...
\index{randdataset@\texttt{randdataset}}
\begin{interactive}
...
all it takes is to run PostgreSQL's \texttt{\idx{initdb}} and installing the set
...
\postgresdocu{functions-srf.html}{set returning functions}\index{set returning function} allows to
...
terminology this property of a function is called \idxsql{IMMUTABLE}
(see the PostgreSQL documentation on
\postgresdocu{xfunc-volatility.html}{Function Volatility Categories}).
\index{function volatility categories}
...
\index{gj.pl@\texttt{gj.pl} (job generator)}
...
\index{js.pl@\texttt{js.pl} (job scheduler)}
\index{feed.pl@\texttt{feed.pl} (batch feeder)}
...
\index{qp2log@\texttt{qp2log}}
\index{m//g@\texttt{m//g} (regex)}
\index{G@\texttt{\ttbackslash G} (regex)}
...
that one can build a very \texttt{lex}/\texttt{flex}-like scanner. \index{lex/flex}
...
\label{sec:pivotlog}
\index{pivot\_log@\texttt{pivot\_log}}
...
the skyline \idx{selectivity} factor\index{selectivity factor}\footnote{selectivity factor = skyline
...
causal relation with the skyline \idx{selectivity} factors.  It shows
...
SQL have been proposed, like \idx{Preference~SQL} \citep{Kiessling2002a}.  A
...
partitions as induced by the \idxsql{PARTITION BY} subclause. This
syntax was inspired by Oracle's analytic functions \citep[Page~5-10]{Oracle2005}.


\subsection{\idxsql{USING} \emph{op} for \inlinesql{SKYLINE OF DIFF}}
\idxonlysql{DIFF}
...
such as 
\emph{Index} \citep{Tan2001}, 
\emph{Nearest Neighbor} (NN) \citep{Kossmann2002}, and
\emph{Branch and Bound} (BBS) \citep{Papadias2003, Papadias2005}.
...
A couple of algorithms for low cardinality domains, e.g.  the ``stars
ranking of a hotel'', only five discrete values (one to five stars),
have been proposed \citep{Preisinger2006, Preisinger2007, Morse2007}.
Based on histograms for the base relations the query optimizer could
select such specialized algorithms.


\subsection{Speedup Tuple Comparison}
We believe the concept of elimination filter (EF) is very promising
and we believe that it is possible to further improve its efficiency
and effectiveness, e.g. to integrate it as a filter in the index scan
code, or to use other data structures and policies for the EF window.
...
In this thesis we present the extension of PostgreSQL with the skyline
operator and extensive experiments, with the ultimate goal of building
a skyline query optimizer.
%
It is our strong belief that with this effort the skyline operator is
in the habitat it belongs to.

First, we give a general introduction and motivate why the name
``skyline operator'' is a good choice.
%
Secondly, we give a formal definition of the skyline operator and we
treat in detail how skyline queries can be rewritten into standard
SQL.
%
Thirdly, we deal with existing non-index-based skyline algorithms.  We
describe them using pseudo-code and show some of their properties,
which are called ``physical properties'' in the database context.
%
We prove that the original version of BNL does not terminate in all
cases and give a corrected version.
%
Furthermore we talk about related works.


In \autoref{chap:implementation} we describe the details of our
implementation.
%
The way the pseudo-code is given fits into the pipelined
architecture of the execution engine.
%
The algorithms we have implemented so far are two specialized
algorithms for one dimensional skyline queries, a special case for two
dimensions (PRESORT), a \naive nested loop algorithm (MNL), BNL, SFS,
and a variant of LESS.
%
We extend the basic syntax of the \inlinesql{SKYLINE~OF}-clause
\citep{Borzsonyi2001} to allow influence on the semantics
(treatment of \texttt{NULL} values, etc.) and operational aspects
(physical operator, tuple window size/policy, and so forth).
...
%
Furthermore we provide a series of patches, applicable against the
PostgreSQL 8.3 stable release.
%
By means of that we hope to acquaint the skyline operator to a larger
audience and lay the ground for further research in the direction of
integrating preference query concepts into RDBMSs.
%
We wish that at least one flavor of SQL preference clauses will make
it into a future SQL standard.

We have conducted extensive experiments with a total workload of more
than 200~days of CPU time on various datasets.
%
The experimental environment we have designed and set up for this task
is noteworthy on its own.
%
...
skyline query is not more than $0.1$; (2) for datasets up to $500$
tuples and of relatively small dimensions (e.g. up to five) BNL
performs the best in all aspects.
%
