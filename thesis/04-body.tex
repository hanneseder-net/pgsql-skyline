...
\todo{see \citep[Page 3]{Godfrey2004}}{}
...
As long as it did not change the sematics of the input or output we
...
Furthermore at certain points we even allowed ourselfs to strip some
...
\todo{http://en.wikipedia.org/wiki/Relational\_model}{}
...
relational database management system (RDBMS)\index{RDBMS} we restrict
...
To give an intuition, \dominates will be a binary relation between
pairs of tuples from the same (database) relation \relation{R} with
schema \schema{R}, we say $r$ \emph{dominates} $s$ in \schema{R} iff
$(r, s) \in \dominates$ (or in infix notation $r \dominates s$).
...
\todo{
In general a partial order is not a \emph{total order}, nor is a
strict partial order a \emph{strict total order}, what distingueses
partial orders ..... we have this in the 1 dim case}{}

\begin{itemize}
\item \emph{totality:} $\forall x, y: x \dominates y \lor y \dominates x$.
\end{itemize}



%\todo{some more properties, which hold}{}
%\begin{itemize}
...
\]
\emph{if and only if} the following condition holds
\begin{eqnarray}\label{equ:skylinepf}
\overbrace{\left( \bigland_{1 \le i \le k} r_i \le s_i \right)}^{\textit{``\/}\texttt{MIN}\textit{ at least as good''}} \,\land\,
...
\todo{make \dominates equal to (\ref{equ:skylinepf}) and \weakdominates equal to just the $\le$ part, use $r \nondistinct s := r \weakdominates s \land s \weakdominates r = \forall i \in \{1, \ldots, m\}\colon r_i = s_i$, i.e. equal on all skyline dimensions}{}

\[
r \incomparable s := r \notdominates s \land s \notdominates r
\]
...
% todo http://en.wikipedia.org/wiki/Strict_weak_order
\todo{}{}
Transitifity of equivalence does not hold for a strict partial order
\[
...
\todo{for a strict total order R the law (or axiom) of trichotomy holds:}{}
\[
\forall x, y\colon x R y \lor x = y \lor y R y
\]


If $\forall i \in \{1, \ldots, m\}\colon r_i = s_i$ holds, $r$ and $s$ are
...
\[
DR_\dominates(r) := \{ x \in \schema R | r \dominates x \}
\]
\end{definition}
...
\subsection{How our implementation extends the mathematical model}
...
possible, nevertheless this might be a good use case for a user
defined ordering operator.
...
see \citep[Page 5, Theorem 4]{Chomicki2002a}
...
\subsection{Rewrite Skyline Queries into Standard SQL}
\label{sec:rewrite-to-sql}

\todo{such queries can not be unnested \citep{Grust1997, Braumandl1998}, from \citep[Page 4]{Borzsonyi2001}}{}

Let $rel$ by a relation with the attributes $a_1, \ldots, a_n$, a non
empty target list $\emptyset \not= target\_list \subseteq \{a_1,
\ldots, a_n\}$, then a skyline query with at least one criterion ($0
\le k \le l \le m \le n$ and $1 \le m \le n$) and following schema:
%
\begin{sql}
...
\end{sql}
%
can be rewritten, as examplified in \citep{Borzsonyi2001}, into
%
...
Note that the attributes specified as a criterion in the skyline query
need not neccessarly appear in the $target\_list$.

The correspondence with the basic defintion of the skyline operator
(cf. Defition~\ref{def:skyline}) and with the defition of the binary
preference releation \eqref{equ:skylinepf} is obvious.

The join is a $\theta$-join and not an equi-join.

A little notion we use here is $condition\{r \gets o\}$ to express
that a any reference to the releation alias $r$ must be replace with
the alias $o$.
...
without those tuples that have a \emph{wittness}\index{wittness} that
...
Computing the skyline in this way is expensive. \todo{}{}

\subsubsection{Rewrite queries with \inlinesql{SKYLINE OF DISTINCT}}
In the presents of the optional \inlinesql{DISTINCT} modifier in the
\inlinesql{SKYLINE~OF}-clause situation is changed, such a query has
following schema:
%
...
intended sematics of \inlinesql{SKyLINE~OF DISTINCT} is
...
\]
i.e. they are equal on all skyline critiria, in other terms $r
\nondistinct s$ (cf. \eqref{eqn:nondistinct}), holds then either $r$
or $s$ is retained, the choice is left to the implementation. Note
that the attributes $a_{m+1}, \ldots, a_n$ are not effected by this,
there are still part of the retained tuple, i.e no implicit projection
is performed. This indicates some difficulties, standard SQL does not
have some non-deterministic feature, so we some how have to express if
a set of tuples coincide on the sklyine criteria $\{a_1, \ldots,
a_m\}$, i.e. $r \nondistinct s$, how to:
...
\noindent
The later is easy, as we just of to eliminate duplicates, which can be
done with the \inlinesql{DISTINCT} keyword in the
\inlinesql{SELECT}-clause.  The rewritten query of
\eqref{sql:rewrite-distinct} is besides an additional
\inlinesql{DISTINCT} the same as in \eqref{sql:rewritten-non-distinct}:
%
\begin{sql}
\newcommand\abox[1]{\hbox to 2.5em{#1\hfil}}%
\newcommand\bbox[1]{\hbox to 2em{#1\hfil}}%
...
Matters get more involved in the first case.  If we can establish a
...
least one tuple.}.  In the presents of a unique and orderable
key\footnote{On some RDBMS special pseudocolums for such proposes do
...
To examplify this, let the target list be $\{a_{j_1}, \ldots,
a_{j_h}\}$, $\{j_1, \ldots, j_h\} = J$, with $J \subseteq \{1, \ldots,
n\}$ and $J \cap \{m+1, \ldots, n\} \neq \emptyset$, i.e the target
list contains at least one attribute not part of the skyline
...
\ttspace{4}AND \abox{$i.a_{l+1}$} { }= \abox{$o.a_m$} AND $\ldots$ AND \bbox{$i.a_m$} { }= \bbox{$o.a_m$} \ttspace{2}-- "DIFF" equal \\
\ttspace{4}AND (\\
...
expression, by means of that duplicates can be eliminate in a way we
...
This returns only the correct result of all the subselects will return
the attribute form the same tuple, which can be expected as the same
...
For the border case of one dimensional skyline queryies, i.e. $m = 1$,
other methods do exist, we show them in
Section~\ref{sec:onedim-nondistinct} and \ref{sec:onedim-distinct}.
...
Furthermore from an relational algebraic point of view, index-based
skyline algorithms can only be placed an the leaves of an algobraic
expresion, where the non-indexed based sklyine algorithms are first
order citizens and can be placed any where in the relational algebra 
expression, so they are full general.
\todo{}{}
...
BNL was implemented according to the algorithm proposed in \citep{Borzsonyi2001}.
...
$\mathbb{W}$ & tuple window in main memory (\emph{type}: set of $d$-dimensional points with a timestamp value for each point and if the window policies \emph{entroy} or \emph{random} are used, then each point has an associated \emph{rank}) \\
...
begin
	...
	while $\neg$EOF($\mathcal{I}$) do begin
		...
		// continue with next pass if necassary
...
\subsubsection{Nontermination of Block-Nested-Loops (BNL) Algorithm}
During the implemention of BNL into PostgreSQL we discovered a flaw in
the algorithm from the original paper \citep{Borzsonyi2001}.  Besides
...
added and than removed if there is no more space the code of
\citep{Borzsonyi2001} for BNL is the same is in Figure~\ref{code:bnl}.
Under certain condition the code from Figure~\ref{code:bnl} does not
terminate.  To show this property, we construct a data set like this:
...
\noindent
where $n$ is number of tuples that fit into the tuple window
$\mathbb{W}$ and $m \ge 1$, with following dominance structure:
...
\ldots, a_n, b_1, \ldots b_m$ anti-correlated and let the $c_i$'s just by
...
the tuple window of size $n$ slots with this tuples as they are all
incomparable
...
tuple window is completly filled with the $c_i$'s tuples
...
is reached, resulting in a switch to the temp file and reseting
$timestampIn$ and $timestampOut$ to zero, no tuple $c_i$ from the
tuple window will be propageted to the output as they have a timestamp
...
The simplest case to reconstruct this nontermination behaviour is with
$n=m=1$.  A possible fix is to duplicate the lines 7--12 between 44
and 45 in Figure~\ref{code:bnl}, so the propagation is done once EOF
is reached.  Nevertheless we fixed it with a slight modification in
the control flow, see Figure~\ref{code:fixedbnl}.  And of course, our
...
\subsubsection{Non-preservation of relative Tuple Order}
\label{sec:bnl-tuple-order}
It is not completly obious that BNL does not preserve the relative
order of tuples from the input in the output, even when the tuple
window policy \emph{append} is used.  To examplify this property
...
the temp file, $a_1$ was eliminated by $c_1$. In the text pass $b_1$
and $c_1$ are compared against each other, but as the are incomparable
...
therefor BNL does not preserve the relative order of tuples.  This
physical property of BNL is important during query planing, as the
relative order of tuples is an important aspect here, e.g. if a prior
operator in the query plan establishes a certain order or preserves it
an explicit sort can be ommited or for instance consider differnt
phyiscal operators for aggregation or joining.

\subsection{Sort First Skyline (SFS)}
The implementation of SFS is according to \citep{Chomicki2003}.  It
differs from BNL in that the data is topologically sorted at start-up
time.  We realize the sorting by utilizing the same physical operator
as for the \texttt{ORDER BY}-clause.
%
...
While the skyline operator is insensitive to the order of attributes,
more indexes can be considered by the query optimizer for the access
path selection.  We modified the query optimizer accordingly.
...
function SkylineSFS($\mathcal{I}$) begin 
	...
	while $\neg$EOF($\mathcal{I}_{presorted}$) do begin
		...
		// continue with next pass if necassary
		if EOF($\mathcal{I}_{presorted}$) then begin
			// switch to tempfile $\mathcal{T}$
...
\subsubsection{Preservation of relative Tuple Order}
...
In \citep{Godfrey2005} proposed the LESS algorithm, an improvement
over SFS \citep{Chomicki2003}. LESS is generally considered as fastest
...
external sort routine toeliminate tuples early, hence less tuples left
to sort; and
\item
...
The concept of elimination Filter (EF) is simply but effective.  It
tries to eliminate as many non-skyline tuples in an early stage.  To
do so it compares every incomming tuple $p$ against a tuple window
...
garantee to filter out all non-skyline tuples, but a skyline candiate
is garanteed to survive the elimination filter.  An other noteable
...
\citet{Godfrey2005} do not give pseudo code for the elimination
filter in there paper. In Figure~\ref{code:ef} we give a sketch for
the code as we used it in our implementation. Note that we are give
the algorithm in a form that fits into a pipelined architecture liked
it is used in the PostgreSQL quey execution engine. The function
...
over calles to \texttt{EliminationFilter} as it keeps the internal
...
conjunction with as SFS style algorithm, in this paper we demonstrate
that this concept is also very effective when used with an BNL style
algortihm, we call it BNL+EF, cf. Section~\ref{sec:analysis}
...
function EliminationFilter($\mathcal{I}$, var $state$) begin 
	while true do begin
		...
		case PROCESS:
			...
			if $isCandidate$ then begin
				if hasfreespace($\mathbb{W}$) then begin
					// add $p$ to the tuple window $\mathbb{W}$
					add($\mathbb{W}$, $p$);
				end
...
In \citep{Godfrey2005} Godfrey et al. proposed the LESS
...
SFS node (see Figure~\ref{fig:qp-sfs-ef}).  Furthermore, we do not
...
\emph{entropy} can only be used if statistics for basetables is
available.
...
to store tuples.} and compare one tuple after the other agains all
other tuples in the tuple store. It is obvious that this algorithm has
$O(n^2)$ run time complexity and very poor I/O behaviour. Nevertheless
...
testing, because this method was just easier to get right and later on
...
PostgreSQL has the feature to use user defined ordering operators for
...
but the create of a the user-defined ordering operator can give it any
name.  We borrowed this concept and integreated it into SQL extension
for the \inlinesql{SKYLINE~OF}-clause, where \inlinesql{MIN} is equivalent
to \inlinesql{USING <} and \inlinesql{MAX} is equivalent to \inlinesql{USING >}.


\citep{Chomicki2002} for red/white wine, meat/fish

see PostgreSQL documentation on
\postgresdocu{sql-select.html\#SQL-ORDERBY}{ORDER BY-clause}
...
\todo{changeset: [657], \texttt{subquery\_is\_pushdown\_safe} @ \srcref{src/backend/optimizer/path/allpaths.c}, If the subquery contains SKYLINE OF we cannot push quals into
 * it, because that would change the results ... FIXME: is it true?}{}
...
LAST}). The information if the sort order is asceding (\texttt{ASC})
...
important role troughout the query plan.  A note within the query plan
might creates, can make use of, or destroys a certain pathkey.  If
created or preserved the nodes higher up in the query plan can make
use of it.  This information is considered by the query planner when
deciding amough different physical operators, e.g. \emph{stream
...
see \srcref{src/backend/executor/README}
\todo{The iterator model in the executor}{}

PostgreSQL query execution engine has a pipelined architecture,
i.e. in general no intermediate results are materialized. \todo{}{}

\todo{It is coded in the fashion of a state machine.  Do somthing in each state and then proceed to another state.  The state is stored in the node's execution state information and is preserved acress calls.}{}


\srcref{src/backend/executor/execProcnode.c} 
...
\begin{interactive}
SELECT * FROM a15d1e4s0 SKLYINE OF d1 MIN, d2 MIN;
\end{interactive}
...
\begin{interactive}
SELECT * FROM a15d1e4s0 SKLYINE OF d2 MIN, d1 MIN;
\end{interactive}
...
\inlinesql{SKYLINE~OF}-clause can versatily a richer set of indexes
(access paths) than the \inlinesql{ORDER~BY}-clause can.  To be
...
Section~\ref{sec:attributeorder}. We modified the query planner
...
\todo{lookup make slyline pathkey, what is the name of the function that mathces a pathkey agains a skyline pathey}
...
\emph{pathkeys}\index{pathkeys} as described in Section~\ref{sec:pathkeys}. As a
fullblown operator we have to provide this information for the skyline
operator es well.  In the Sections~\ref{sec:bnl-tuple-order},
\ref{sec:sfs-tuple-order}, and \ref{sec:ef-tuple-order} in subsections
respectivly, we bring forward arguments why BNL does \emph{not}
...
case and for our na\"{\i}ve algorithms it is easy to see that they
preserve the relative tuple order.  The properties are summariezed in
Table~\ref{tab:tuple-order-preserving}. Note that for the
...
\texttt{skyline\_methode\_can\_use\_limit}\footnote{in \srcref{src/backend/utils/skyline/skyline.c}}
...
Figure~\ref{fig:qp-agg-bnl-sort}).
...
\todo{explain ExecRestrPos, ExecMarkPos, ExecReScan, ExecProcNode}{}
...
\todo{write about slot, resultSlot that they are preserved over calles
write about how pseudo code is related to the real implementation,
we changed names an removed some nasty details}{}

\subsection{Special case: one dimensional}
\label{sec:onedim-nondistinct}
Altought this case is of limited use, the general case algorithms like
...
to passes over the input, first finding the minimum by means of
...
approach corresponds to a na\"{\i}ve nested-loop, the query plan is
accordingly:

\todo{maybe skip this plan}{}
\begin{interactive}
                                    QUERY PLAN                                   
---------------------------------------------------------------------------------
...
cf. Figure~\ref{code:1dim}.  For the sake of a clearer pseudo code we
gave our self the freedom to present the tuple store interface
(\texttt{tuplestore\_*}) a little different to the actual
implementation.  A functionalty we added to the tuple store is to
...
modifing read and write pointers.

\begin{figure}[htbp]
\begin{lstlisting}[language=pseudo]
function ExecSkyline_1Dim(var $state$)
begin
	switch $state.status$ begin
	case INIT:
	...
	case PIPEOUT:
		if $\neg$tuplestore_eof($state.tuplestore$) then
...
input, see Figure~\ref{code:1dimdistinct}. A skyline query of this type looks
like:
...
\noindent
Please note that again two scans over the input are done, altought the
...
\noindent
An adhoc experiment that we conducted showed that Microsoft SQL Server
2005 produces a very similar execution plan.

In Oracle the pseudocolumn \inlinesql{ROWNUM}\index{ROWNUM@\texttt{ROWNUM}}
...
select the first, accoring to the order induced by \texttt{id}, of the
...
begin
	...
	case INIT:
		...
		if $resultSlot \not=$ NULL then begin
			...
			// while not end of input reached
			while $slot \not=$ NULL do begin
				if $slot \dominates resultSlot$ then
					$resultSlot \gets slot$;
...
then then the skyline can be computed with a single scan over the
input, while keeping just the most recently found skyline tuple, see
Figure~\ref{code:2dimpresort}.  Of course in the presents of an
...
\todo{}{give reference to semantics of distinct keyword}
In the pseude code in Figure~\ref{code:2dimpresort} \texttt{DISTINCT}
...
becomes the next skyline if the following condintion holds:
...
begin
	...
	case PROCESS:
		forever do begin
			...
			if $slot\ =$ NULL then begin
				// end of input reached
				$state.status \gets$ DONE;
				return NULL;
			end

			if $slot \dominates returnSlot \lor (slot \nondistinct returnSlot\ \land$ DISTINCT$)$ then begin
...
dimensionalty and treat the \inlinesql{DISTINCT} modifier.  In
directly corresponds to the na\"{\i}ve $O(n^2)$ nested loop approach,
compare every tuple to all others as long no wittness is found, which
...
index scan.  The handle the \inlinesql{DISTINCT} case, we just use a
	...
	case PROCESS:
		forever do begin
			...
			forever do begin
				...
				if $innerSlot\ =$ NULL then begin
					// no wittness found
					return $slot$;
				end

				if $innerSlot \dominates slot$ then
					break;
				else if $innserSlot \nondistinct slot \land$ DISTINCT then begin
					// favor the first
					if $innerPos < state.pos$ then
						break;
				end
			end
		end
...
skyline (candiate) points. As internal representation we used a double
linked list with a sentinel (cf. \citep[Page 204--209]{Cormen2002}).
...
KBs or if a a maximum of \lstinline|maxSlosts| slots. The number of
slots is the stronger limit. For policy see
Section~\ref{sec:tuplewindowpolicies}.  The returned value must be
used in all subsequent calles to the tuple window interface functions.

\item \lstinline|void tuplewindow_end(TupleWindowState *state)| 
destroys a tuple window and frees all recources.
...
this eases the implementation and an exising PostgreSQL interface the
...
tuple is needed.  We use this especially for the implemention of the
elimination filter. We speak of a \emph{foreced insert}\index{forced
insert}.  Currently the \lstinline|timespace| argument is only used
...
orginized in the window and in which order the are compared against
...
\item \emph{entropy:}
tuples are keep in \emph{rank descanding order}. The inserted position
...
\item \emph{random: }
belaves like \emph{entropy} but instead of a an computed entropy value
...
\todo{fix overfull page}{}
\begin{figure}[htbp]
\begin{lstlisting}[language=pseudo] %, caption={Pseudo code for BNL}, label={code:fixedbnl-statemachine}]
function ExecSkyline_BlockNestedLoop(var $state$) begin forever do
begin switch $state.status$ begin case INIT: $state.source \gets$
OUTER; tuplestore_init($state.tempOut$);
tuplewindow_init($state.window$); $state.tsIn \gets 0$; $state.tsOut
\gets 0$; $state.status \gets$ PROCESS; break;

		case PROCESS:
			if $state.source$ = OUTER then
				$slot \gets$ ExecProcNode(outerPlanState($state$));
			else
				$slot \gets$ tuplestore_get($state.tempIn$);

			if $slot =$ NULL then begin
				if $state.source$ = TEMP then
					tuplestore_done($state.tempIn$);

				if $state.tsOut = 0$ then begin
			...
			tuplewindow_rewwind($state.window$);
			if $state.ranked$ then
				tuplewindow_setinsertrank($state.window$, ExecSkylineRank($state$, $slot$);

			forever do begin
				if tuplewindow_ateof($state.window$) then begin
					if tupelwindow_hasfreespace($state.window$) then
						tuplewindow_put($state.window$, $slot$);
					else begin
						tupelstore_put($state.tempOut$, $slot$);
				...
				if $innerSlot \dominates slot \lor (innerSlot \nondistinct slot \land \text{DISTINCT})$
					break;
				else if $slot \dominates innerSlot$ then
		...
		case PIPEOUT:
			while $\neg$tuplewindow_ateof($state.window$) do begin
				if tuplewindow_timestampcurrent($state.window$) = $state.tsIn$ then
		...
		case FINALPIPEOUT:
			if $\neg$tuplewindow_ateof($state.window$) then
				return tuplewindow_get($state.window$);
...
\subsubsection{Presort+EF (for 2 dim only)}
...
Section~\ref{sec:totalworkload} for the total computational workload
...
buffers. See Section~\ref{sec:experimentalenvironment} for more
details.
...
\begin{itemize}
\item 
\emph{sequencial scan vs. index scan} SFS can benefit from index scan
(ommit sort phase), BNL and SFS could benefit from a index scan if
DIFF groups are used.
...
\begin{itemize}
\item 
use 90\% percentile (\citep{Gray1993}[page 315]) vs, average /
variance (box-plots?)

\item
describe software (operating system: Windows XP + SP 2, Microsoft
Visual Studio 2005 + SP1, based on PostgreSQL 8.3.0) and hardware
(CPU, RAM, Disk config, File System) used for benchmarking

\item
describe the configuration of PostgreSQL (flags used for
\texttt{configure --enable-asserts --enable-checks, ...}, it is
recommended to turn asserts and co. off for benchmarking
\end{itemize}
...
experiments is interessting on its own and therefore we sketch it in
this section.
%
...
other. To distigues these terms from the general terms with the same
...
Figure~\ref{fig:jobfile}, \ref{fig:logfile}
...
of a query was of interesst for us, when we did regression testing to
ensure our implementation is correct. To accomblish this we prefixed
...
A Setup Query is a query whos soly purpose is to setup things up for
...
why larger than our biggest relation we the assoziated indexes this
was always the case.
...
Setup Query (cf. Figure~\ref{fig:jobfile}).
...
Section~\ref{sec:jobscheduling}) and is stored in a \texttt{.sql} file
and typically looks like displayed in Figure~\ref{fig:jobfile}.
...
\comment{For the query sections all relevant information from the query plan is parsed into CSV format with the columns \texttt{runid}, \texttt{name} and \texttt{value}.  See Table~\ref{tab:queryplanfields} for a describtion of the name/value pairs.\commentskip{}}
...
#--</query>
\end{interactive}
\caption{A log file parsed with \texttt{qp2log}.  For the name and describtion of the extracted fields see Table~\ref{tab:queryplanfields}.}
...
We also used Windows XPs \texttt{sysprep} during the clone process.

For interprocess communication we used file sharing and we use file
...
described in Table~\ref{tab:dirlayoutexpiriments}.
...
\verb|  prep/|    & \\
\verb|    gj.pl|  & job generator, see Section~\ref{sec:generatejobs} \\
...
\verb|      ws/|  & This one for example holds the jobs, when we were
                    analyzing the effect of varing window size.\\
\verb|      .../| & \\
\verb|    s1/|    & \\
\verb|    s2/|    & \\
...
Section~\ref{sec:generatejobs}) in the \texttt{prep/} directory. We
distributed the Jobs to serveral directories below
\texttt{prep/}, namely to \texttt{s0}, \texttt{s1} and \texttt{s2} and
there subdirectories. Using \texttt{xcopy /m} makes this task easy, as
...
\texttt{done/}. See Section~\ref{sec:jobscheduling} for more details.
As a result of the Job being executed a raw log file
(cf. Figure~\ref{fig:logfile}) is generated in the \texttt(log/)
directory. We parse this raw log files to CSV format with comment
lines (cf. Figure~\ref{fig:parsedlogfile}) with a script called
\texttt{qp2log}, see Section~\ref{sec:tocsvformat}. The last step
before aggregating and analyzing the data (see
Section~\ref{sec:aggregatingandanalyzing}) is to pivot and project the
CSV files to a more compact representation
(cf. Figure~\ref{fig:pivot_log}) by script called \texttt{pivot\_log},
see Section~\ref{sec:pivotlog}.
...
We will speak of it as \emph{job scheduling}.  The concept behind this
...
processor \texttt{psql}. The ouptut is redirected to a log file in the
...
function to read the content of a directory with thousends of
\texttt{*.sql} files, when you actually just want to pick one of
them. Globbing a dirctory with thousends of files could take longer
than running one of these jobs and instead of running them in
parallel, they are processed serially.  Instead rely \texttt{opendir},
...
little bit more interessted in some results than in some others.  The
...
There is more little thing to tell, \texttt{js.pl} and
...
By means of this simple arragement of files, directory and scripts we
...
the queries executation plan are listet in
Table~\ref{tab:queryplanfields}.  We would like to point out a feature
of Perl's regex support which was very handy for us, it is the
combination of ``\texttt{m//g}'' and ``\texttt{\ttbackslash
G}''. ``\texttt{\ttbackslash G}'' matches only at the end-of-match
position of the prior \texttt{m//g}.  By means of that one can build a
very \texttt{lex}/\texttt{flex} like scanner.
...
For any node type in the executation plan we output these quantities.
The node names are mapped to \texttt{skyline}, \texttt{elimfilter},
\texttt{seqscan} and \texttt{idxscan}.  Although other node types do
exist in general, they have not been of interesst in our studies.
...
are in an arbitrary unit and that the actual cost are in milliconds,
see PostgresSQL documentation on
...
\verb|[SKYLINENODE].method|		& Currently one of \texttt{1dim}, \texttt{presort}, \texttt{mnl}, \texttt{bnl}, \texttt{sfs} and \texttt{elimfilter}. NOte there are actually differnt methodes for 1 dimensional distinct and non-distinct, but we distingues them only by the next field here.\\
...
\verb|seqscan{.ALIAS}.on|		& Name of the basetable, when an alias is used for the table it is given as \texttt{.ALIAS}, like when we express a skyline query only with SQL, we have an inner \texttt{.i} and an outer \texttt{.o} relation.\\
\verb|seqscan{.ALIAS}.tablesize|	& Here we do a little hack and derive this value from the tablename (\# rows). \\
...
\verb|total|				& The total runtime of the query in millisecondes (ms).\\
\end{tabular*}
...
(cf. Figure~\ref{fig:parsedlogfile}) into CSV format, where each Run
is represented by a single line and values for the selected names are
the columns (cf. Figure~\ref{fig:pivot_log}). For a list of fields
with describtion see Table~\ref{tab:queryplanfields}.
...
savers and so and so forth (cf. section~\ref{sec:lessonlearned}).  The
...
Figure~\ref{fig:all-dim-onek} depicts the time-dimensionality
...
distributions).  As illustrated by Figure~\ref{fig:sf}, there exists a
...
Figure~\ref{fig:all-dim} as well.
...
four to five representative dimension values.  Figure~\ref{fig:inde},
...
(Figure~\ref{fig:inde}).  As the dimension is higher than four, SFS is
...
selectivity factor for independent datasets in Figure~\ref{fig:sf},
...
Section~\ref{sec:small-datasets}.  Note that the same observation
...
(cf. Figure~\ref{fig:sf-c}).  Except for the data cardinality of 100
...
Figure~\ref{fig:qp-sfs-index-ef}.  It is up to the query optimizer to
...
Figure~\ref{fig:sfs-index-a} when dim=6).  This is clearly an argument
for using an index.

However, SFS+EF w/ index performs worse than SFS w/ index.  This is
somehow counterintuitive, because for 100k tuples SFS+EF performs
better than SFS, as we have discussed in
Section~\ref{sec:big-datasets} (cf. Figure~\ref{fig:all-dim}).  We
...
append, and dim $\le 3$ (cf. Figure~\ref{fig:sfs-index-c}).  We argue
this is due to the general low selectivity factor and high EF
effectiveness in this case (cf. Figure~\ref{fig:sf-c} and
\ref{fig:ef-eff-100k}).
...
in the window.  Figure~\ref{fig:ef-eff} shows that \emph{entropy} is
...
\section{Differnt syntax for \inlinesql{SKYLINE OF DIFF}}
We believe the syntax of \inlinesql{SKYLINE OF DIFF} is a bit
cumbersom for the intended semantic to partition the input relation
into partitions, so that the skyline is computed with these
partitions. Instead of \inlinesql{DIFF} a construct like this could be
used:
...
For SFS differenciate between CMP\_INCOMPAREABLE\_SAME\_GROUP and
CMP\_INCOMPAREABLE\_DIFFERENT\_GROUP.

\section{Project Tuples before inserting into SFS Tuple Window}
...
to be copied and manupilated.  Some cost estimation or some heuristics
could be developt when to perform this projection.

\section{Heuristics}
\subsection{Flush Elimination Filter}
\todo{}{}
If the elimination filter does not eliminate a certain amout of tuples
...
\inlinesql{SKYLINE OF DIFF}, which is used to partion the input
