...
over SFS \cite{Chomicki2003}, where the essential differences between
SFS and LESS are:
...
\begin{figure}[htbp]
\begin{lstlisting}[language=pseudo]
program EliminationFilter() begin 
	$\mathbb{W} \gets \emptyset$;
	while $\neg$EOF($\mathcal{I}$) begin
		$p \gets$ read($\mathcal{I}$);
		$isCandidate \gets true$;
...
\end{lstlisting}
\caption{Pseudo code for Elimination Filter (EF) as filter program}
\label{code:efprogram}
\end{figure}
...
sort phase, as described in \cite{Godfrey2005}.  Therefore,
...
Note that only $d$ dominates $b$ ($d \dominates b$) and that $a, c,
\text{and} d$ are skyline points.

\begin{tabular}{lcccccc}
Iteration              & $p$ & Window $\mathbb{W}$ & tsIn & tsOut & Temp $\mathcal{T}$ & $\mathcal{O}$ \\
\hline
0 (Init)               &     & $\emptyset$          & 0 & 0 & $\emptyset$ & $\emptyset$\\
1                      & $a$ & $\{a\;(0)\}$         & 1 & 0 & $\emptyset$ & $\emptyset$\\
2                      & $b$ & $\{a\;(0), b\;(0)\}$ & 2 & 0 & $\emptyset$ & $\emptyset$\\
3                      & $c$ & $\{a\;(0), b\;(0)\}$ & 3 & 1 & $\{c\}$     & $\emptyset$ \\
4                      & $d$ & $\{a\;(0), d\;(1)\}$ & 4 & 1 & $\{c\}$     & $\emptyset$\\
4 (Load $\mathcal{T}$) &     & $\{a\;(0), d\;(1)\}$ & 0 & 0 & $\emptyset$ & $\emptyset$\\
5 (Propagate)          &     & $\{d\;(1)\}$         & 0 & 0 & $\emptyset$ & $\{a\}$ \\
5                      & $c$ & $\{d\;(1), c\;(0)\}$ & 1 & 0 & $\emptyset$ & $\{a\}$ \\
5 (Load $\mathcal{T}$) 
