...
to express the \emph{block structure}, like in
\href{http://www.python.org/}{Python} or similar languages.
...
\noindent
Furthermore we took the freedom to simply some function names and
...
\texttt{nodeToString}/\texttt{stringToNode} (serialise/deserialise
to/from text).
...
the PostgreSQL documenation for the \postgresdocu{parser-stage.html}
...
the definition of \texttt{skyline\_clause}.

\section{Query Analyzing}
In this phase, which is also described in PostgreSQL documentation on the
\postgresdocu{parser-stage.html}{parser stage}, the raw parse tree
from the previous stage is semantically analyzed.  Lookups into the
system catalog are done in this stage to resolve references to tables,
columns, functions, and operators, see
\srcref{src/backend/parser/parse\_clause.c}
\texttt{transformSkylineClause}.

\section{Query Rewriting}
For our implemenation of the sklyline operator it was not necessary to
modify the \emph{query rewriting} stage.  For details see
documentation on the \postgresdocu{rule-system.html}{PostgreSQL Rule
System}.
