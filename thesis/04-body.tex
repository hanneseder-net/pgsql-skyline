...
\todo{make \dominates equal to (\ref{equ:skylinepf}) and \weakdominates equal to just the $\le$ part, use $r \nondistinct s := r \weakdominates s \land s \weakdominates r = \forall i \in \{1, \ldots, m\}: r_i = s_i$, i.e. equal on all skyline dimensions}{}
...
\[
\neg \forall x, y, z: x \incomparable y \land y \incomparable z \imp x \incomparable z
\]
...
\[
\forall x, y: x R y \lor x = y \lor y R y
\]
...
A skyline query with following schema:
%
\begin{sql}
SELECT * FROM $rel$ AS $r$ WHERE $condition$ \\
SKYLINE OF $a_1$ MIN$, \ldots, a_k$ MIN$, a_{k+1}$ MAX$, \ldots, a_l$ MAX$, a_{l+1}$ DIFF$, \ldots, a_m$ DIFF
\end{sql}
%
can be rewritten into \citep{Borzsonyi2001}
%
\begin{sql}
\newcommand\abox[1]{\hbox to 2.5em{#1\hfil}}%
\newcommand\bbox[1]{\hbox to 2em{#1\hfil}}%
SELECT * FROM $rel$ AS $o$ WHERE ($condition\{r \gets o\}$) AND NOT EXISTS \\
...
(cf. Equation \todo{reference}{}) and with the defition of the binary
preference releation (cf. Equation~\ref{equ:skylinepf}) is obvious.

The join is a $\theta$-join and not an equi-join.

\todo{in the condition replace r with o resp. i}{}

Another way to rewrite the query is \citep[Page 3]{Godfrey2004}:

\begin{sql}
\newcommand\abox[1]{\hbox to 2.5em{#1\hfil}}%
\newcommand\bbox[1]{\hbox to 2em{#1\hfil}}%
SELECT * FROM $rel$ WHERE ($condition$) \\
EXCEPT \\
SELECT $o.$* FROM $rel$ AS $i$, $rel$ AS $o$\\
...
\todo{execution plan for sql server}{}
