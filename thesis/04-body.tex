...
%done:manos: I didn't see the traveling example anywhere in the text
%hannes:let's use the standard example here
...
%done:manos: I'm not sure, but I think that the correct term in this case
%       is equivalence (and equivalence class) since equality is
%       supposed to express cases where the = sign holds.
%hannes: you are right
\subsubsection{\inlinesql{DIFF}-Induced Equivalence Relation}
...
\bigskip
\noindent
Using the monotone increasing functions  $f_1$ and $f_2$ to define a
monotone scoring function $S(r) = f_1(r_1) + f_2(r_2)$:
...
\noindent
we show that is possible to give the tuple $(2,2)$ a higher score
than $(4,1)$ and $(1,4)$:
...
%hannes: hm? might skip that ``ranking'' foo
...
%manos: It might be a good idea to keep the pseudo-code of the original bnl and
%the fixed bnl each in their own page so that the reading and comparison is
%easier (plus it will make it more readable since the tabs are not aligned from
...
coincide.
%done:fang: this sentence is too long, I cannot understand it.
...
semantics, this definition gave us great flexibility during the
...
%done:fang: please rewrite this sentence
...
The knowledge about this is encoded in PostgreSQL my means of so
...
using an index scan.
\emph{Pathkeys} are essentially a list each member of which is a \emph{pathkey},
...
must not be flattened out, i.e. pull up the subquery.  The function
...
The physical operator selection, also called as \emph{method}, is
rather straightforward.  If a method is specified in the skyline options,
...
Unfortunately neither question is easy to answer, and it is at least the case for the
...
suitable access path the input need not to be sorted, performing an
...
get the tuple at the current position of the cursor.  When called with
...
\item \emph{entropy:}
tuples are keep in \emph{rank descending order}. The insertion position
...
\subsection{SFS}
Listing \ref{code:sfs} translates straight forward into a state machine,
...
and continue to debug.  Kind of deluxe for a C compiler.} was possible.  For further
...
or optimizing and/or refactor other parts.
...
\texttt{tuplestore}'s-\texttt{work\_mem}\index{tuple store} gets full, since for parts
...
\verb|    s0/|    & To this and to the directories below we copied the
                    ...
                    there no jobs left here.  This is done by
...
RDBMS\index{RDBMS} PostgreSQL with the skyline operator.  In our strong believe with
