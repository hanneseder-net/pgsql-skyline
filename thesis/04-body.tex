...
\item \todo{ask Ali, which website for traveling by airplane uses skyline}{}
...
\autoref{fig:vienneseskyline} might come to ones mind, the
...
buildings. \autoref{tab:threeDskyline} shows the coordinate of the
...
For sure building $c$ (the green in the background) and building $f$
...
\item \todo{easier to spec. that a correlated query / SQL with except}{}
\item \todo{faster to evaluate}{}
\item \todo{new optimization opportunities}{}
\end{itemize}
...
\todo{}{}
\todo{We consider only BNL, SFS and variants}{}
...
depend on the properties of the data sets such as dimension,
...
\todo{}{}
The paper is organized as follows: In \autoref{sec:algorithms} we
introduce briefly the implemented skyline algorithms.
\autoref{sec:configuration} describes the experimental setup.
\autoref{sec:analysis} presents the extensive experiments over
various data settings, followed by the analysis of the
results. \autoref{sec:conclusion} concludes the paper and
...
everything finite} domains: $N$ (numbers) and $D$ (uninterpreted
...
strict partial order a \emph{strict total order}, what distinguishes
...
In case condition \eqref{equ:skylinepf} does not hold $r$ and $s$ are
...
anti dominance region is empty. This can be done with a range query
even facilitating an index.}{}
...
According to the definition in 5.2 \citep{Chomicki2002} the skyline
...
scoring functions (see \autoref{subsec:scoring}).
...
a set of tuples coincide on the skyline criteria $\{a_1, \ldots,
a_m\}$, i.e. $r \nondistinct s$, how to:
...
\todo{FLET(Fast Linear Expected-time), [BCL90]:}{}
...
expresion, where the non-indexed based skyline algorithms are first
...
and the \texttt{GROUP BY} node (see \autoref{fig:qp-agg-bnl-sort}),
...
\citep{Borzsonyi2001} for BNL is the same is in \autoref{code:bnl}.
Under certain condition the code from \autoref{code:bnl} does not
...
and 45 in \autoref{code:bnl}, so the propagation is done once EOF
...
algortihm, we call it BNL+EF, cf. \autoref{sec:analysis}
...
see \autoref{sec:distinct}. For the entire skyline clause we
introduced just a single new reserved keyword, see
\autoref{sec:reservedkeywords}.
...
the sementics of this see \autoref{sec:nullvalues}.
...
SQL, is parsed into a data structure.  In this section we describe this
process for the grammar of the \inlinesql{SKYLINE OF}-clause as defined
lin \autoref{sec:sql-extension}




see \srcref{src/backend/parser/gram.y} and 
\srcref{src/backend/parser/keywords.c}
...
results see \autoref{sec:analysis-window-size}.
...
In the pseude code in \autoref{code:2dimpresort} \texttt{DISTINCT}
	...
	forever do begin
		...
		case PROCESS:
			...
			if $slot =$ NULL then begin
				...
				if $\neg state.neednextrun$ then begin
					// nothing written to temp -> done
					tuplestore_done($state.tempOut$);
					tuplewindow_rewind($state.window$);
					$state.status \gets$ DONE;
			...
			forever do begin
				if tuplewindow_ateof($state.window$) then begin
					if tupelwindow_hasfreespace($state.window$) then begin
						tuplewindow_put($state.window$, $slot$);
						return $slot$;
					end
					else begin
...
cf. \autoref{code:ef}. 
\todo{To complete the picture we describe here 
the options that can be specified.}{?}

\subsubsection{SFS+EF (a LESS Variant)}

\subsubsection{BNL+EF}
...
We started out our implemention efford of the skyline operator, when
\texttt{CSV HEAD} of the PostgreSQL source repository was
...
in our SVN repostory, including the CVS metadata directories
\texttt{CVS}.  By means of that we continiously merged \texttt{CVS HEAD}
...
von Version 8.3.0 of PostgreSQL with our patch for the skyline
operator applied.
...
\begin{itemize}
\item \texttt{cvs-entiers-normalize.sh:}
sorts the content of the \texttt{CVS/Entries*} files, to have a
normalized form, and minimize the changes that are commited to the
SVN repository.

\item \texttt{resetcvsts.pl:}
sets the file date/time according to the values in \texttt{CVS/Entries},
this allowes to easly move around the working copy accross different machines.
\end{itemize}
...
PostgreSQL creates one backend process, which corresponds to an
...
queries directly and the query is evaulated in the same process.  So
debugging or with Microsoft Visual C++ even \emph{edit \&
continue}\index{edit \& continue}\footnote{Edit \& continue is the
ability of modifing the source code while debugging, apply the changes
...
To fasilitate regression testing we also integrated a test case in
...
and this would be appriciated as part of a checked build.  For the
...
\autoref{sec:set-returning-function}.

We ran both, the PostgreSQL regression test suite and our Perl script
\texttt{slregress.pl}, very frequently during development, to ensure that
we do not break other parts of PostgreSQL and an already implemented
feature of the skyline operator implementation, while we were
integrating new parts or optimize and/or refactor other parts.
...
\comment{For the query sections all relevant information from the query plan is parsed into CSV format with the columns \texttt{runid}, \texttt{name} and \texttt{value}.  See \autoref{tab:queryplanfields} for a describtion of the name/value pairs.\commentskip{}}
...
#--</query>
\end{interactive}
\caption{A log file parsed with \texttt{qp2log}.  For the name and describtion of the extracted fields see \autoref{tab:queryplanfields}.}
...
with describtion see \autoref{tab:queryplanfields}.
...
savers and so and so forth (cf. \autoref{sec:lessonlearned}).  The
...
% LocalWords:  tex skyband todo mathescape morekeywords foreach nightshot Kung
% LocalWords:  Pflügl Borzsonyi Preparata threeDskyline vienneseskyline nissan
% LocalWords:  istockphoto BNL SFS Chaudhuri PostgreSQL RDBMS tuples tuple EF
% LocalWords:  NOINDEX psql Simkovics Torlone Ciaccia Chomicki equ skylinepf
...
% LocalWords:  skylinepfdiff xindex XINDEX OPFAMILY VARCHAR nba
