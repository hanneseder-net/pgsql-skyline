...
\begin{sqlscript}
\prompt{CREATE DATABASE SKYLINE;}
...
\begin{figure}[htbp]
\centering
\subfloat[Density Plot]{\includegraphics[width=100mm]{plots/density-2d-i2d1e5}}
\subfloat[Statistics]{
\begin{tabular}{r|rr}
	& d1 	& d2 \\
\hline
...
\begin{figure}[htbp]
\centering
\subfloat[Density plot]{\includegraphics[width=100mm]{plots/density-2d-c2d1e5}}
\subfloat[Statistics]{
\begin{tabular}{r|rr}
	& d1 	& d2 \\
\hline
...
\begin{figure}[htbp]
\centering
\subfloat[Density Plot]{\includegraphics[width=100mm]{plots/density-2d-a2d1e5}}
\subfloat[Statistics]{
\begin{tabular}{r|rr}
	& d1 	& d2 \\
\hline
...
\begin{sqlscript}
\prompt{SELECT * FROM generate_series(2,4);}
 generate_series
-----------------
               2
               3
...
\begin{sqlscript}
\prompt{SELECT * FROM
pg_rand_dataset('indep', 2, 10, 0) AS
ds(id int, d1 float, d2 float);}
 id |         d1         |         d2
----+--------------------+--------------------
...
\prompt{SELECT * FROM
pg_rand_dataset('corr', 3, 10, 0) AS
ds(id int, d1 float, d2 float, d3 float);}
 id |        d1         |        d2         |        d3
----+-------------------+-------------------+-------------------
...
\prompt{SELECT * FROM
pg_rand_dataset('anti', 3, 20, 0) AS
ds(id int, d1 float, d2 float, d3 float);}
\comment{output ommit, as it similar to above, except other distribution and more records}
\end{sqlscript}
...
\begin{sqlscript}
\prompt{SELECT * FROM rds2d('indep', 10, 0);}
\comment{output ommited, as it is as above}

\prompt{SELECT * FROM rds3d('corr', 10, 0);}
\comment{same here}

\prompt{SELECT * FROM rds3d('anti', 20, 0);}
\comment{same here}
\end{sqlscript}
...
\begin{sqlscript}
\prompt{SELECT * FROM generate_series(1,1000000) LIMIT 1;}
 generate_series
-----------------
               1
(1 row)
...
\begin{sqlscript}
\prompt{SET function_scan_work_mem  = 102400;}
\end{sqlscript}
