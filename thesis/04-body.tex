...
operator} \citep{Chomicki2003a}, we define it in a very similarg way.
...
term database relation in the usual sens, a (database) relation
\relation{R} a finite instance a schema \schema{R}.
...
$\weakdominates$ are formalized like this:
\begin{itemize}
...
\noindent
If a strict partial order has also the property of
...
Since this close relationship between the first-order formula $C$ and
...
formula depends on other tuples, like in \emph{``we prefere tuples
that are better than the average''}, which involes aggregation, it
...
We now like to define a special form of preference formula.
...
\todo{the syntax is not yet part of a standard %
%
