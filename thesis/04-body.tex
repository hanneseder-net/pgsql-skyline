...
Since the close relationship between this first-order formula and the
binary relation we use notations $\skyline_\dominates$ and
$\skyline_C$ interchangeably.
...
\noindent
Please note that in general $r=s$ is not equal to $r \nondistinct s$
especially when $m < n$, i.e. there are some extra attributes not
subject to skyline computation.

%We say $s$ is dominated by $r$ iff $r$ dominates $s$, formally:
\noindent
For convenience we define following two \emph{commutator} relations:
\[
...
\begin{definition}[Compareable]The tuples are $r$ and $s$ are \emph{comparable} iff $r$ dominates $s$ or $s$ dominates $r$:
\[
...
scoring functions (see \autoref{sec:scoring}).  The term skyline stratum
appeares in \citep{Chan2005}, but probably was also early works.
%
...
enjoy to little bit longer walk we would have to take in that case.
...
It's might be tempting to see a direct relationship between
...
but $\skyband_\dominates^0 = \{r, s\}$, $\skyband_\dominates^1 = \emptyset$ and
$\skyband_\dominates^2 = \{t\}$.

\noindent
Hence no easy general relation can be established between K-skyband
and skyline stratum, so following holds:
...
skyline startum are computed until at least $k$ tuples are found, or
...
It should also be noted that it might be difficult to come up with
appropriate weights for the different attributes from a users
perspective.

% example:
% \citep[page 6, near theorem 4]{Chomicki2002a}
...
nevertheless the selectivity of this approach is even lower then
