...
quality and a cheap product with lower quality.  For sure we are not
...
another}, as there are others which better fulfill our criteria.  The
...
operator.  In \autoref{chap:relatedworks} we introduce briefly the
...
implementation. \autoref{sec:results} describes the experimental setup
...
discussed in \autoref{chap:future-work}. \autoref{chap:summary}
concludes the paper.
...
We assume two infinite\footnote{Although in a real computer is
...
\begin{definition}[preference formula (pf) \citep{Chomicki2002}]
A \emph{preference formula (pf)} $C(r,s)$ is a first-order formula
defining a preference relation $\succ$ in the standard sense, namely
\[
r \succ s \textnormal{iff} C(r,s).
\]
\end{definition}

\begin{definition}[winnow operator \citep{Chomicki2002}]
If $\schema R$ is a relation schema and $C$ a preference formula defining
a preference relation $\succ$ over $\schema R$, then the \emph{winnow operator}
is written $\omega_C(\schema R)$, and for every instance $\relation R \in \schema R$:
\[
...
\begin{definition}[Preference Relation and Preference Formula]
%\noindent
...
The first order formula $C(r,s)$ is called \emph{preference formula}.
Since the close relationship between this first order formular and the
binary relation we use notations $\skyline_\dominates$ and
$\skyline_C$ interchangable.
...
\noindent
Using the above two definition formula \eqref{equ:skylinepf}
simplifies to:
...
\noindent
For convinience we define following two \emph{commutator} relations:
\[
...
\begin{definition}[Incomparable] If neither $r$ dominates $s$ nor $s$ dominates $r$ than $r$ and $s$ are \emph{incomparable}:
\[
...
\noindent
The commutativitiy of $\incomparable$ is obvious from its definition, i.e.
\[
r \incomparable s = s \incomparable r.
\]


\begin{definition}[Compareable]The tuples are $r$ and $s$ are \emph{comparable} iif $r$ dominates $s$ or $s$ dominates $r$:
\[
...
In the context of skyline operator the following two notions naturally
arise:

\begin{definition}[Dominance Region]
The Dominance Region $DR_\dominates(r)$ of a data point $r$ in the
schema $\schema R$ is the set of all possible data points in $\schema
R$ that are dominated by $r$, formally:
\[
...
The Anti-Dominance Region $ADR_\dominates(r)$ of a data point $r$ in the
schema $\schema R$ is the set of all possible data points in $\schema R$
that dominate $r$, formally:
\[
ADR_\dominates(r) := \{ x \in \schema R | x \dominates r \}
\]
\end{definition}
...
\end{property}
\begin{proof}
This property immediatly becomes clear, when looking at the definition
of the \emph{preference formula}, see \eqref{equ:skylinepf}, as
logical and ($\land$) and logical or ($\lor$) or commutative.
\end{proof}
...
skyline startum are computed till at least $k$ tuples are found, or
the entire relation is returned.  Nevertheless it should be noted that
the selectifity of the top-$k$-skyline operator is even less
...
\begin{theorem}[\citep{Chomicki2002a}]The skyline contains all, and only
tuples yielding maximum values of monotone scoring functions.
\end{theorem}
...
Computing the skyline in this way is expensive, as the intermediate
results could get very large.
...
key\footnote{On some RDBMS special pseudo-columns for such proposes do
...
There is a whole class of other skyline algorithms, referenced as
\emph{continuous skyline computation}.  
The task is here the initially compute the skyline and that update the
...
and BNL will make an other pass.  The tuples in the tuple window get a
timestamp to be able the decide which tuples in the window have been
...
paper that this condition is sufficient for following property.  In
SFS once a tuple is incomparable to all tuples in the tuple window it
is know to be a skyline tuple as well, and it can be written to the
...
The closeset related works to this paper are \citep{Chaudhuri2006}
and \citep{Goncalves2005, Goncalves2005a}.  Authors of both papers did an
implementation of the skyline operator into an RDBMS as a fullblown
relational operator.

\citet{Chaudhuri2006} at Mircorsoft Research extended a Beta
Version of Microsoft SQL Server 2005.  To the best of our knowdledge
...
\citep{Goncalves2005} extended PostgreSQL with an hyprid approach 
...
nevertheless the selectifity of this approach is even lower then
...
value. A prerequirement for this to work, is that the $12$ uniformly
...
To let the skyline operator be a fullblown relational operator one has
...
also \autoref{sec:operatorselection, sec:cardinality-and-cost-estimation}.
...
\section{\inlinesql{USING} \emph{op} for \inlinesql{SKYLINE OF DIFF}}
...
questions, sampling could be engaged. \citet{Chaurduri2006} do
