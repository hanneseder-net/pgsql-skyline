...
\[
r \succ s \quad \textnormal{iff} \quad C(r,s).
\]
\end{definition}
...
\noindent
If $r \nondistinct s$, in case of \inlinesql{SKYLINE OF DISTINCT} it
is left up to the imlementation to include either $r$ or $s$ in the
...
\subsubsection{\inlinesql{DIFF}-Induced Equaltity Relation}
...
Before going in to detail let us define the class of linear
scoring functions:

\begin{definition}[Linear Scoring Function]
Let the $w_i$'s be positive real constants, then we define a 
\emph{postive linear scoring function} $W$ over a the tuples 
$r \in \relation R$ as:
\[
W(r) = \sum_{1 \le i \le k} w_i r_i.
\]
\end{definition}
...
function that preferes $(4, 1)$ or $(1,4)$ is obvious.  Now let us try
...
0$.  Therefore there is not lineare scoring function which gives
$(2,2)$ the best score.
...
\bigskip
\noindent
On the other hand the class of monotone scoring function is defined as follows:

\begin{definition}[Monotone Scoring Function]
Let the $f_i$'s be monotone increasing functions with $f_i\colon N
\rightarrow \mathbb{R}$, where $N$ is domain of the attributes of
...
One of the nice conclusion of \autoref{the:skyline-monotone} is, no
matter how we weight our preferences along the attributes, our
favorite is part of the skyline, as long we weight each attribute with
a monotone increasing function.  Furthermore only tuples which
monotone score best under at least one scoring function are part of
