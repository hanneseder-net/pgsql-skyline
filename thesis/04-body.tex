...
\subsection{Pathkeys}
\todo{we could describe it here as we use it more at a couple of placed below?}{}
...
\noindent
produce the same result. SKLYINE OF d2 MIN, d1 MIN;Clearly the same is not true in general for
...
PostgreSQL term to express this property is called \emph{pathkeys},
they arise naturally in conjunction with an access path, e.g.  when
scanning a tuple via a clustered index or using an index scan.
...
As a fullblown operator we have to provide this information for the
skyline operator es well.  In the Sections~\ref{sec:bnl-tuple-order},
