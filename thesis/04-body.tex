...
\todo{http://en.wikipedia.org/wiki/Relational_model}{}
...
\item \texttt{--<query runid=}\textit{runid}\texttt{>} and \texttt{--</query>}; A Run cf. definition below.  Please not that the corresponding Setup Queries are not nested within the Run they just precede the Run.
...
described in Table~\ref{tab:tab:dirlayoutexpiriments}.

\begin{table}[!ht]
\centering
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}p{2.2cm}@{\extracolsep{\fill}}p{\textwidth-2.2cm}}
\verb|jobs/|      &  \\
...
We will speak of it as \emph{job scheduling}.  The concept behind this is
simply but very effective. We run a processes called \emph{job
...
\todo{list in a table the values which are collected?}{}
