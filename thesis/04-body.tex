...
\subsubsection{\inlinesql{SKYLINE OF DISTINCT} is generally \emph{not} expressible in SQL}
...
Matters get more involved in the first case. We would like to
distigues two subcases. First if the relation $rel$ has an orderable
attribute which uniquelly identifies a tuple, such as \emph{rowid} are
an other key let use call it \texttt{id}\footnote{On some RDBMS system
special pseudocolumns might be used for this propose.} and be one of
the $\{a_{m+1}, \ldots, a_n\}$ attributes, then
\eqref{sql:rewrite-distinct} can be rewritten into:
%
\begin{sql}
\newcommand\abox[1]{\hbox to 2.5em{#1\hfil}}%
\newcommand\bbox[1]{\hbox to 2em{#1\hfil}}%
...
\ttspace{4}AND \abox{$i$.id} { }< \abox{$o$.id}\ttspace{14} -- pick the first\\
\ttspace{3})\\
\ttspace{2})\\
\ttspace{1})
\end{sql}
%
In the above query we retain the first tuple, with an easy principle
of we have a wittness with a lower \texttt{id} we drop the later
tuple. Also note all the addition pairs of paranthesis neccessary.

If the attributes not part of the skyline criteria are orderable, one
could establish a lexikographic order, pick the smallest as above with
\texttt{id} and to ensure no duplicates are left do a duplicate
elimintition over the entire query.

The second subcase is the more difficult one, where we do not have a
unique and orderable identifier for a tuple.  Let the target list be
$\{a_{j_1}, \ldots, a_{j_h}\}$, $\{j_1, \ldots, j_h\} = J$, with $J \subseteq \{1, \ldots, n\}$ and $J \cap \{m+1, \ldots, n\} \neq \emptyset$, i.e
the target list contains at least one attribute not part of the
skyline criteria. In Microsoft SQL Server there are special aggregate
functions \inlinesql{FIRST} and \inlinesql{LAST} one can use in this
case, the rewritten query of \eqref{sql:rewrite-distinct} is as:
%
\begin{sql}
\newcommand\abox[1]{\hbox to 2.5em{#1\hfil}}%
\newcommand\bbox[1]{\hbox to 2em{#1\hfil}}%
...
%Or in Oracle \inlinesql{HAVING GROUP\_ID() < 1}
%(cf. \citep{Oracle2005}) could be fascilitated, but 
%^^^ thats not true, because all fields have to be part of the aggregate
In the absence of such a feature or a feature that can be used to
accoblish such a task, like there is nothing in the SQL 2003 standard
\citep{SQL2003} the skyline query is \emph{not} expressible.

\todo{
select (select id from foo as i where d1 = o.d1 limit 1), (select d2 from foo as i where d1 = o.d1 limit 1) from (select distinct d1 from foo) as o;
}{}
\todo{credit to chris :) }{}

For the border case $m = 1$, i.e. one dimensional skyline, other
methods do exist, we show them in Section~\ref{sec:onedim-nondistinct}
and \ref{sec:onedim-distinct}.  Nevertheless the \inlinesql{DISTINCT}
case relies on SQL features not part of the SQL 2003 standard and are
...
\begin{figure}[htbp]
\begin{lstlisting}[language=pseudo]
function SkylineBNL($\mathcal{I}$) begin // initialization
...
\todo{talking about the methods you have implemented.}{}
...
skylines for the two dimensional case can be computed in a simple way; first ... \todo{}{}

\todo{in case a suitable access path, no sort is needed,}{}

\todo{brauchbar, da es unserer vermutung nach ein ein gut anzuwendender fall
ist, aber d=2 macht skyline sinn.}

In interesting aspect of this algorithm is how it deals with the
distinct and non-distinct case of \inlinesql{SKYLINE OF}.
\todo{}{}
$q \dominates p \lor (q \nondistinct p \land \text{DISTINCT})$
